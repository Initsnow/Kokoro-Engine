...
- **데스크탑 플로팅 모드**: Live2D 모델을 투명한 플로팅 창으로 데스크탑에 표시——항상 최상위, 테두리 없음, 완전 투명.
  - 우클릭 드래그로 창 이동, 우클릭 메뉴로 크기 조절 모드 진입 후 가장자리 드래그로 자유롭게 크기 변경.
  - Ctrl + 스크롤 휠로 모델 스케일 세밀 조정.
  - 글로벌 단축키(키 녹화로 커스터마이즈 가능)로 채팅 입력창 호출, AI 답변은 모델 위 별도 말풍선 창에 표시.
  - 위치, 창 크기, 모델 스케일은 재시작 후 자동 복원.

### 🧠 AI 두뇌
- **멀티모달 대화**: **Ollama**(로컬) 및 **OpenAI 호환 인터페이스**(클라우드)를 대화의 핵심으로 지원합니다.
- **멀티모달 능력**: Vision 모델을 통합할 수 있으며, **스크린샷**, **이미지 업로드** 또는 **웹캠 실시간 비전**을 지원하여 캐릭터가 내용을 실시간으로 "보고" 설명할 수 있습니다. 웹캠 비전은 다중 장치 선택과 실시간 미리보기를 지원하며, 매 채팅 메시지에 최신 카메라 프레임을 자동으로 첨부합니다.
...
- [ ] **Linux 및 macOS 지원**: 현재 Windows에서만 충분히 테스트되었습니다. Linux 및 macOS에서의 완전한 기능 검증 및 최적화가 필요합니다.
